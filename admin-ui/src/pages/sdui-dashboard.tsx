...
export function SduiDashboardPage() {
  const { data: layout, isLoading, isFetching, error } = useLayout("dashboard");
  const store = getPageStore("dashboard");
  ...
  return (
    <SduiAdminShell>
      {showSpinner ? (
        ...
        >
          <LayoutRenderer layout={layout} pageId="dashboard" store={store} />
        </div>
      ) : null}
    </SduiAdminShell>
  );
