...
export function SduiExtensionsPage() {
  const { data: layout, isLoading, isFetching, error } = useLayout("extensions");
  const store = getPageStore("extensions");
    ...
    <SduiAdminShell>
      {showSpinner ? (
        ...
        <div className={isFetching ? "opacity-90 transition-opacity" : undefined}>
          <LayoutRenderer layout={layout} pageId="extensions" store={store} />
        </div>
      ) : null}
    </SduiAdminShell>
  );
