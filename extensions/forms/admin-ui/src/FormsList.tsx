...
import { Plus, Search, MoreVertical, Edit2, Trash2, FileText, ExternalLink } from "lucide-react";

const { Button, Card, CardContent, Badge, Table, TableBody, TableCell, TableHead, TableHeader, TableRow, Input } = (window as any).__VIBECMS_SHARED__.ui;
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-500">Manage your contact forms and custom data collection points.</p>
                </div>
                <Button onClick={() => navigate("/admin/ext/forms/new")} className="bg-indigo-600 hover:bg-indigo-700">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="hover:bg-transparent border-slate-100">
                                ...
                                <TableHead className="text-slate-500 font-medium">Shortcode / Slug</TableHead>
                        ...
                        <TableBody>
                            {loading ? (
                                <TableRow>
                                    <TableCell colSpan={4} className="h-32 text-center text-slate-400">Loading forms...</TableCell>
                                </TableRow>
                            ) : filteredForms.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={4} className="h-32 text-center text-slate-400">No forms found.</TableCell>
                                </TableRow>
                            ) : filteredForms.map((form) => (
                                <TableRow key={form.id} className="group border-slate-100 hover:bg-slate-50/50 transition-colors">
